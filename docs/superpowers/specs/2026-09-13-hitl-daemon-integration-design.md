# Design: HITL Local Exec — Production Daemon Integration

## 1. Context & Objective

`docs/hitl_prd.md` proposes a Streaming Interceptor Hook inside the production
...
`docs/superpowers/specs/2026-09-13-hitl-dev-chat-design.md` implemented a first
...
a complete HTTP round-trip, and HITL detection scans a round's already-finished
...
- New daemon-level setting `hitlEnabled: boolean` in `AppConfig` (`src/config.ts`),
  persisted the same way `mode` is, set via a new `--hitl` startup flag on
  `src/cli.ts`/`src/server.ts` (parallel to the existing mode flags).
- HITL is honored only when **both** hold at daemon startup:
...
- If `--hitl` is requested but either condition is false, the daemon logs a
  clear warning and runs with HITL disabled for the entire process — fail
...
`src/dev-chat/hitl-protocol.ts`, `hitl-approval.ts`, and `hitl-exec.ts` move
verbatim to a new shared location `src/hitl/` (`protocol.ts`, `approval.ts`,
`exec.ts`), with no behavior change. `src/dev-chat/` re-exports from `src/hitl/`
...
The production daemon integration consumes `src/hitl/*` directly; it does not
depend on anything in `src/dev-chat/`.

## 5. The exec gate: a new `hitlExecGate` hook in `browser-worker.ts`
...
- `turn.hitlExecGate` is only ever set when `config.hitlEnabled` (§3); absent,
...
**`HitlExecGate.check`'s implementation** lives in a new, much smaller module,
`src/adapters/chatgpt-web/hitl-interceptor.ts`:
`createHitlExecGate(deps): HitlExecGate`, where
`deps = { approvalGateway, runCommand: typeof runApprovedCommand, workspaceCwd }`
(all from `src/hitl/*`, unchanged). `check(finalText)` calls
`parseExecRequest(finalText)` (`src/hitl/protocol.ts`); on no match, returns
`{ action: "finalize" }`; on a match, calls
`runCommand(approvalGateway, request, workspaceCwd)` (`src/hitl/exec.ts`,
...
1. `turnTimeoutMs`'s deadline check (`:4364-4366`, `:4791-4793`) is a hard,
   no-grace `throw`. When `config.hitlEnabled`, the adapter passes
   `turnTimeoutMs: undefined` for HITL-active turns (no deadline), matching how
...
2. `ChatGptTurnDomHealthTracker`'s 60-second "text present, no completion
   ...
   reuses that same suspension signal — while `hitlExecGate.check(...)` has an
   ...
   unaffected by this suspension — it already runs *before* `hitlExecGate` is
...
only when `config.hitlEnabled`: it accumulates `text_delta` text since the last
...
normally once `hitlExecGate`'s resume/finalize decision produces further
...
timing lives in §5's `hitlExecGate`, not here. When `config.hitlEnabled` is
...
is modified only as described in §5 (a new optional `hitlExecGate` field and
loop branch, gated entirely behind its presence).

## 7. Execution mechanics

Identical to the dev-chat design, reusing `src/hitl/exec.ts` unchanged:
...
- `src/cli.ts` / `src/server.ts`: new `--hitl` startup flag, help text update,
...
- The daemon logs the interceptor's approval prompts to its own attached
  ...
  reused via `src/hitl/approval.ts`) — this is the terminal the operator
  attached when starting the daemon in foreground/`--hitl` mode, not Codex's
  own UI.
...
- `src/hitl/*`: move existing `src/dev-chat/hitl-*.test.ts` files to
  `tests/hitl-*.test.ts` unchanged (import paths updated), plus a
  `tests/dev-chat.test.ts` regression pass to confirm the re-export move
  didn't change dev-chat's own behavior.
- `tests/hitl-exec-gate.test.ts` (new): drive `createHitlExecGate(...).check(text)`
  ...
  formatting from `src/hitl/*` (unchanged, already covered by their own tests —
  this test only checks the gate's own translation into `HitlExecGate`'s
  return shape).
- `tests/hitl-emit-filter.test.ts` (new): drive the §6 emit-wrapping filter
...
- `tests/browser-worker-hitl-gate.test.ts` (new, extending the existing
  `browser-worker-contract.test.ts` test-double harness): exercises
  `runBrowserTurn`'s new loop branch directly — a `hitlExecGate` that returns
  ...
  `hitlExecGate.check(...)` is pending.
- One test extending the existing `chatgpt-web` adapter test suite exercising
  `index.ts`'s `!mode.localTools` branch with `hitlEnabled: true`, using a fake
  browser-worker double: confirms `turn.hitlExecGate` is wired to
  `createHitlExecGate(...)` and `turnTimeoutMs` is passed as `undefined`.
- No changes to `bridge.ts`'s own test suite, since that file is unmodified.
  ...
  lives in the new `browser-worker-hitl-gate.test.ts` file to keep the new
...
- No changes to `src/server.ts`'s request-processing/SSE logic beyond the
  new `--hitl` startup flag plumbing (§3, §8); no changes at all to
  ...
  design adds a new, independent `hitlExecGate` hook alongside them rather
  than reusing or modifying them.
- `src/adapters/chatgpt-web/browser-worker.ts` gains exactly one new optional
  field (`hitlExecGate` on `BrowserTurn`) and one new loop branch gated behind
  it (§5); every other line of that file's behavior is unchanged when
  `hitlExecGate` is absent, which is the case for every non-HITL turn (all of
  `full` mode, and any `browser-only` turn without `--hitl`).
- No change to `full` mode, the MCP connector, or the OpenAI Tunnel — HITL
  remains exclusively a `browser-only`-mode feature (per the approved scope
  decision; the PRD's framing of HITL as a full tunnel/MCP replacement is
  explicitly out of scope here).
- No Electron/launcher UI approval surface (interface left ready for one, as
  in the dev-chat design).
- No support for HITL in a detached/headless daemon process — activation
...
- No changes to the dev-chat CLI's own behavior beyond importing shared
  protocol/approval/exec modules from their new `src/hitl/` location.
